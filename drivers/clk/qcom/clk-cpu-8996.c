...
static struct clk_fixed_factor cbf_pll_postdiv_pro = {
	.mult = 1,
	.div = 4,
	.hw.init = &(struct clk_init_data){
		.name = "cbf_pll_postdiv",
...
static const struct clk_parent_data cbf_mux_parents[] = {
	{ .fw_name = "xo" },
	{ .hw = &cbf_pll.clkr.hw },
...
static struct clk_regmap_mux cbf_mux = {
	.reg = CBF_REG_OFFSET + CBF_MUX_OFFSET,
	.shift = 0,
	.width = 2,
	.clkr.hw.init = &(struct clk_init_data) {
		.name = "cbf_mux",
		.parent_data = cbf_mux_parents,
		.num_parents = ARRAY_SIZE(cbf_mux_parents),
		.ops = &clk_regmap_mux_closest_ops,
...
static int qcom_cpu_clk_msm8996_register_cbf_clks(struct device *dev,
						  struct regmap *regmap)
{
	int ret;
	bool is_pro = of_device_is_compatible(dev->of_node, "qcom,msm8996pro-apcc");

	ret = devm_clk_hw_register(dev, is_pro ? &cbf_pll_postdiv_pro.hw : &cbf_pll_postdiv.hw);
	if (ret)
		return ret;

	ret = devm_clk_register_regmap(dev, &cbf_pll.clkr);
	if (ret)
		return ret;

	ret = devm_clk_register_regmap(dev, &cbf_mux.clkr);
	if (ret)
		return ret;
	...
	/* Set CBF PLL rate and enable it */
...
static int qcom_cpu_clk_msm8996_driver_probe(struct platform_device *pdev)
{
	...
	ret = qcom_cpu_clk_msm8996_register_cbf_clks(dev, regmap_cbf);
	if (ret)
		return ret;
