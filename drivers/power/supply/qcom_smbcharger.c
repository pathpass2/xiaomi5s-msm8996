...
static int smbchg_usb_get_ilim(struct smbchg_chip *chip)
{
	...
	if (!(value & USBIN_MODE_HC_BIT)) {
		ret = regmap_read(chip->regmap,
				  chip->base + SMBCHG_USB_CHGPTH_CHGPTH_CFG,
				  &value);
		if (ret)
			return ret;
	...
	/* AICL */
	ret = regmap_read(chip->regmap,
				chip->base + SMBCHG_USB_CHGPTH_ICL_STS_1,
				&value);
	if (ret) {
		dev_err(chip->dev,
			"Failed to read ICL status: %pe\n",
			ERR_PTR(ret));
		return ret;
	}

	return chip->data->ilim_usb_table[value & ICL_STS_MASK];
}
...
static int smbchg_usb_set_ilim_hc(struct smbchg_chip *chip, int current_ua)
{
	...
	ret = smbchg_sec_masked_write(chip, SMBCHG_USB_CHGPTH_AICL_CFG,
...
static int smbchg_usb_aicl_enable(struct smbchg_chip *chip)
{
	...
	if (ret) {
		dev_err(chip->dev,
			"Failed to clear ICL override: %pe\n", ERR_PTR(ret));
		return ret;
	}
	...
	* affect the live input current limit if HC mode is enabled.
	*/
	ret = smbchg_sec_masked_write(chip, SMBCHG_USB_CHGPTH_IL_CFG,
				      USBIN_INPUT_MASK,
				      chip->data->ilim_usb_table_len - 1);
	if (ret) {
	...
	/* Enable AICL */
	return smbchg_sec_masked_write(chip, SMBCHG_USB_CHGPTH_AICL_CFG,
...
static irqreturn_t smbchg_handle_usb_source_detect(int irq, void *data)
{
	...
	if (usb_present && (usb_type == POWER_SUPPLY_USB_TYPE_DCP ||
		...
		/*
		 * AICL runs only when the USB charge path is enabled.
		 * limit input current to 100000uA using LC mode then enable
		 * the USB charge path. This limit should be safe to for all
		 * USB ports.
		 * Switching to LC mode here will make sure that the live
		 * input current limit does not change to an unsafe level
		 * during the process of enabling AICL. 
		 */
		ret = smbchg_usb_set_ilim_lc(chip, 100000);
		if (ret < 0) {
			dev_err(chip->dev,
				"Failed to set initial USB current limit: %pe\n",
				ERR_PTR(ret));
			return IRQ_NONE;
		}
...
static irqreturn_t smbchg_handle_aicl_done(int irq, void *data)
{
	...
	/* Make sure HC mode is enabled to make the AICL result effective */
	ret = regmap_update_bits(chip->regmap,
	 			chip->base + SMBCHG_USB_CHGPTH_CMD_IL,
...
static int smbchg_init(struct smbchg_chip *chip)
{
	...
	/* Enable APSD */
	ret = smbchg_sec_masked_write(chip, SMBCHG_USB_CHGPTH_APSD_CFG,
				      USB_CHGPTH_APSD_EN, USB_CHGPTH_APSD_EN);
	if (ret) {
		dev_err(chip->dev,
			"Failed to enable APSD: %pe\n", ERR_PTR(ret));
		return ret;
	}

	/* Enable periodic AICL rerun on the USB charge path */
	ret = smbchg_sec_masked_write(chip, SMBCHG_MISC_TRIM_OPT_15_8,
				      AICL_RERUN_MASK, AICL_RERUN_USB_BIT);
	if (ret) {
		dev_err(chip->dev,
			"Failed to enable AICL rerun: %pe\n", ERR_PTR(ret));
		return ret;
	}
	...
	{
		dev_err(chip->dev,
			"Failed to enable AICL: %pe\n", ERR_PTR(ret));
		return ret;
	}

	/* Set float voltage */
	ret = smbchg_set_vfloat(chip, chip->batt_info->voltage_max_design_uv);
	if (ret) {
		dev_err(chip->dev,
			"Failed to set float voltage: %pe\n", ERR_PTR(ret));
		return ret;
	}
